...
@RestController
@RequestMapping(path = "/payment/refund",
...
public class RefundController {
	...
	public ResponseEntity<RefundResponse> doRefund(@NotNull @Valid @RequestBody final RefundRequest refundRequest) {
		...
		try {
			final TransactionResponse transactionResponse = refundService.processRequest(refundRequest.getOrderId(), refundRequest.getCaptureId(),
																						 refundRequest);
			...
			throw new RuntimeException("Error processing refund request ",e);
		}
